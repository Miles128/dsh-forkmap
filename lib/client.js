window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const NS = "forkmap";
    const STYLE_ID = "dsh-forkmap/client.css";
    const DICTS = {
      zh: {
        "view.map": "地图",
...
.forkmap-canvas { display: block; min-width: 100%; }
.forkmap-edge { fill: none; stroke: var(--ds-border, rgba(27, 31, 36, 0.18)); stroke-width: 1.5; }
.forkmap-node {
  cursor: pointer;
}
.forkmap-node rect {
...
.forkmap-node:hover rect {
  stroke: var(--ds-accent, #4d6bfe);
  stroke-width: 1.5;
}
.forkmap-node.is-active rect {
  fill: var(--ds-bg, #fff);
  stroke: var(--ds-accent, #4d6bfe);
  stroke-width: 2;
}
.forkmap-node.is-running rect { stroke: #3b82f6; }
.forkmap-node.is-pending rect { stroke: #d97706; }
.forkmap-title {
...
.forkmap-fork rect { fill: transparent; }
.forkmap-fork text {
  font: 600 10px/1 ui-sans-serif, system-ui, sans-serif;
  fill: var(--ds-accent, #4d6bfe);
}
.forkmap-fork:hover text { text-decoration: underline; }
body[data-ds-dark-theme] .forkmap-node rect { fill: #232324; stroke: rgba(255,255,255,0.12); }
body[data-ds-dark-theme] .forkmap-node.is-active rect { fill: #2c2c2e; }
body[data-ds-dark-theme] .forkmap-title { fill: #f3f4f6; }
body[data-ds-dark-theme] .forkmap-edge { stroke: rgba(255,255,255,0.16); }
`;
    ...
    function MapView(props) {
      ...
      if (!forest.nodes.length) {
        return h("div", { className: "forkmap" }, h("p", { className: "forkmap-empty" }, t("empty")));
      }
      ...
      for (const node of forest.nodes) {
        ...
        if (!a || !b) continue;
        edges.push(h("path", { key: `${parent.id}->${node.id}`, className: "forkmap-edge", d: edgePath(a, b) }));
      }
      ...
      const cards = forest.nodes.map((node) => {
        ...
        const status = statusLabel(node, t);
        const ago = node.active ? "" : timeAgo(node.updatedAt);
        const meta = [status, ago].filter(Boolean).join(" · ");
        return h(
          "g",
          {
            key: node.id,
          ...
          h("text", { className: "forkmap-title", x: 12, y: 22 }, title),
          meta ? h("text", { className: "forkmap-meta", x: 12, y: 40 }, meta) : null,
          h(
            "g",
            {
              className: "forkmap-fork",
              onClick: (event) => {
                event.stopPropagation();
    ...
    function apply(ctx) {
      if (ctx.locale?.register) {
        ctx.effect(() => ctx.locale.register(NS, DICTS), "forkmap: dictionaries");
      }
    ...
    exports.__testing = { buildMapForest, computeForestLayout, timeAgo };
